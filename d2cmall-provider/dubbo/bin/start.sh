...
if [ -z "$SERVER_NAME" ]; then
    SERVER_NAME=`sed '/app.name/!d;s/.*=//' conf/conf/app.properties | tr -d '\r'`
	  SERVER_PORT=`sed '/dubbo.protocol.port/!d;s/.*=//' conf/conf/app.properties | tr -d '\r'`
fi
...
nohup java $JAVA_OPTS $JAVA_MEM_OPTS $JAVA_DEBUG_OPTS $JAVA_JMX_OPTS -classpath $CONF_DIR:$LIB_JARS -Ddubbo.properties.file=$DEPLOY_DIR/conf/dubbo.properties -Ddubbo.shutdown.hook=true com.alibaba.dubbo.container.Main > $STDOUT_FILE 2>&1 &
